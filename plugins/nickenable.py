        ...
        if len(line[1].split(" ")) < 2:
            ...
            try:
                if line[1].split(" ", 1)[0].lower() not in self.nickmask_disables:
                    self.nickmask_disables[line[1].split(" ", 1)[0].lower()] = set()
                if line[1].split(" ", 1)[1] in self.nickmask_disables[line[1].split(" ", 1)[0].lower()]:
                    self.nickmask_disables[line[1].split(" ", 1)[0].lower()].discard(line[1].split(" ", 1)[1])
                try:
                    ...
                    with open('nickmask_disables.txt', 'wb') as f:
                        pickle.dump(self.nickmask_disables, f)
                self.send(c, target, line[1].split(" ", 1)[1] + " enabled for " + line[1].split(" ", 1)[0])
